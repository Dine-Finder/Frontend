...
const createCustomIcon = (content) => {
  return L.divIcon({
    className: "custom-green-marker", // Apply a specific class and define styles in CSS
    html: `<div class="bg-green-500 opacity-80 text-white border-2 border-green-700 rounded-full inline-flex justify-center items-center text-lg font-bold w-[30px] h-[30px] text-center" style="transform: rotate(29deg);">${content}</div>`,
    iconSize: [30, 30],
    iconAnchor: [15, 15],
    popupAnchor: [0, -15]
  });
...
const createUserIcon2 = (content) => {
  return L.divIcon({
    className: "bg-orange-500 text-dark-gray border-2 border-dark-gray rounded-full inline-flex justify-center items-center text-lg font-bold w-[20px] h-[20px] text-center",
...
const hoverGeoStyle={
  color: "royalblue", //hover border borough
  weight: 2,
  opacity: 1,
  fillColor: "#1C1B22",
  fillOpacity: 0.7
...
const MapComponent = ({info, showing, page, currentPage, selectedPreferences, preferences, updatePreferences}) => {
  ...
  const onEachNeighbourhood = (feature, layer)=>{
    ...
    if (feature.properties.ntaname === selHoodRef.current && selectedPreferences['location']['locationType']=== "Neighborhood"){
      ...
      >
        <MapContainer 
          ...
          url="https://{s}.tile.osm.org/{z}/{x}/{y}.png"
            ...
            {( preferences['location']['locationType']=="Address" && preferences['location']['coordinates'][0]!=null && preferences['location']['coordinates'][1]!=null)
              ?
              <>
              <Marker 
                key="User"
              ...
              :<>
                <GeoJSON data={borough} style={blackOut}/>
                        ...
                        {( selectedPreferences['location']['locationType']=="Address" && selectedPreferences['location']['coordinates'][0]!=null && selectedPreferences['location']['coordinates'][1]!=null)
              ?
              <>
              <Marker 
                key="User"
                icon={createUserIcon("")}
                position={[selectedPreferences['location']['coordinates'][0],selectedPreferences['location']['coordinates'][1]]}
                zIndexOffset={10*(showing+1)}
              />
              <Circle
                center={{lat: selectedPreferences['location']['coordinates'][0], lng: selectedPreferences['location']['coordinates'][1]}}
                color='orange'
                opacity={1}
                weight={2}
                fillColor='purple'
                fillOpacity={0.5}
                radius={selectedPreferences['location']['radius']*1600}
                setZIndex={10}
              />
